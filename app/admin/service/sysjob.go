...
func (e *SysJob) RemoveJob(c *dto.GeneralDelDto) error {
	...
	if err != nil {
		logger.Errorf("msgID[%s] db error:%s", msgID, err)
		return err
	}
...
func (e *SysJob) StartJob(c *dto.GeneralGetDto) error {
	...
	if data.JobType == 1 {
		...
		if err != nil {
			logger.Errorf("msgID[%s] jobs AddJob[HttpJob] error:%s", msgID, err)
		}
	} else {
		var j = &jobs.ExecJob{}
		j.InvokeTarget = data.InvokeTarget
		...
		data.EntryId, err = jobs.AddJob(j)
		if err != nil {
			logger.Errorf("msgID[%s] jobs AddJob[ExecJob] error:%s", msgID, err)
		}
	}
	if err != nil {
		return err
